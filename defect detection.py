...
for index, row in df.iterrows():
    ...
    for names in labelnames:
        ...
        label = file.readline()
	positive_labels = sum(int(label.strip()) for label in labels if label.strip().isdigit())
         if positive_labels / len(labels) >= 0.9:
                    # 이 조건을 만족할 때만 treshold_data에 추가
                    ...
                    print("save csv")
                    break  # 현재 타임스탬프에 대한 라벨 처리가 완료되면 루프 종료
